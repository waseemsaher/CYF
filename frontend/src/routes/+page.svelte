...
<main>
  <header class="site-header">
    ...
    <nav aria-label="التنقل الرئيسي"><a href="/courses">الدورات</a></nav>
  </header>
  ...
  <footer class="site-footer"><span>FCAI COURSES</span><span>تعلم. طبّق. تقدّم.</span></footer>
</main>

<style>
  :global(:root) { --storm: #0f282f; --cyan: #02eff0; --ink: #12343b; --muted: #597078; --paper: #f3f7f6; --line: #d7e5e3; --deep-cyan: #17777a; }
  :global(body) { background: var(--paper); color: var(--ink); font-family: 'IBM Plex Sans Arabic', Tahoma, sans-serif; margin: 0; }
  :global(*) { box-sizing: border-box; }
  .site-header, .hero, .principles, .featured, .site-footer { margin-inline: auto; max-width: 1180px; }
  .site-header { align-items: center; display: flex; justify-content: space-between; padding: 1.5rem 1.25rem 3rem; }
  .brand { color: var(--storm); font-size: 1.05rem; font-weight: 900; letter-spacing: 0.08em; text-decoration: none; }
  .brand span { color: var(--deep-cyan); font-size: 0.68rem; margin-inline-start: 0.35rem; }
  nav a, .section-heading a { color: var(--deep-cyan); font-weight: 800; text-decoration: none; }
  ...
  .primary-action { background: var(--cyan); color: var(--storm); display: inline-flex; font-weight: 900; gap: 0.75rem; padding: 0.85rem 1.2rem; text-decoration: none; }
  ...
  .course-card { background: white; border: 1px solid var(--line); color: var(--ink); display: grid; gap: 1rem; grid-template-rows: auto 1fr auto; min-height: 220px; padding: 1.25rem; text-decoration: none; transition: transform 160ms ease, border-color 160ms ease; }
  ...
  .notice { background: white; border: 1px solid var(--line); padding: 1rem; }
  .site-footer { border-top: 1px solid var(--line); color: var(--muted); display: flex; justify-content: space-between; padding: 1.25rem; }
  @media (max-width: 760px) { .site-header { padding-bottom: 2rem; } .hero { grid-template-columns: 1fr; margin-inline: 1rem; margin-bottom: 4rem; } .hero::after { right: -8rem; } .principles { grid-template-columns: 1fr; padding-bottom: 4rem; } article { grid-template-columns: 2rem 1fr; } article p { grid-column: 2; } .course-row { grid-template-columns: 1fr; } .site-footer { margin-inline: 1.25rem; } }
</style>
